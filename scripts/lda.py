...
class LDA:
    ...
    def __init__(self, docs, freq="bow", topics_count=0, start=2, step=3, stop=30, saved=None):
        ...
        if saved is None:
            ...
            self._model.save(os.path.join("resources/models", f"model_{datetime.now().strftime('%Y%m%d_%H%M%S')}"))
        else:
            self._model = LdaModel.load(os.path.join("resources/models", saved))
